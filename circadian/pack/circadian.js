...
function preload() {
 ...
 table = loadTable('http://0.0.0.0:8000/circadian/circadian.csv', 'csv', 'header');
...
function draw() {
  ...
  fill(178, 143, 206);
      ...
      textFont(font, 14);
      text('LIGHT-SQM READING as Brightness Clock ' +  float(table.getString(r1, 6)),  width*.35, height*.6, 100, 200); 
             fill(255);
         strokeWeight(.5);
...
push();
  ...
  theta += 0.01;
        ...
        strokeWeight(.5);
        //noStroke();
         text('TWITTER MOOD applied as Frequency(HZ) to Oscilator Sine Wave and Analysed with FFT', width*0.02, height*0.89, 450, 400); 
       ...
       if(frameCount%13==1)
        {
          ...
          anger_count = table.getString(r1, 9)*10;
          joy_count = table.getString(r1, 10)*10;
