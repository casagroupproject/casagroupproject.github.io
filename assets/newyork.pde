...
int num[]={139,159,140,155,141,114,117,119,119,89,98,100,82,70,63,58,70,61,172,127,91,98,119,125};
...
int sunmoon[]={1,2,3,4,5,5,4,3,2,1,1,2,3,4,3,2,1,1,2,3,4,3,2,1};
void setup() { 
...
for(int j=0;j<181;j=j+5)
{fill(255);
textSize(10*g);
text(j,5*g,(340-(j*y+20))*g);}
//y
text("NUM",0*g,(340-((j+5)*y+20))*g);
}
...
if(k==1060&i==1060){k=0;i=0;setup();draw();}
 ...
 if(k<=nt){stroke(sun);
   fill(sun);
  ellipse(150*g,60*g,50*g,50*g);}
  if(k>=nt&k<=dt){stroke(bcolor);
fill(bcolor);
rect(150*g,60*g,60*g,60*g);
stroke(255,255,0);
fill(255,255,0);
